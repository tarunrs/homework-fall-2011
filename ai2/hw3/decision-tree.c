...
//    int total_tuple_in_parent = indexes.size();
    ...
    for(col = 0; col < 9; col++){
        ...
        for(int i = 0; i < choices[col]; i++){
          //cout << "Entropy for column " << col << " choice " << i << " : " << entropies.at(i) << endl;
//            if(entropies.at(i))
            weighted_sum_of_entropies += ( (double) temp_indexes[i].size() / size ) * entropies.at(i);
        }
        //cout << "Entropy for column " << col << " : " << weighted_sum_of_entropies << endl;
...
void construct_tree(tree_node * parent, set<int> used_columns){
    ...
    if (homogeneous != -1){
        ...
        //cout << "Decision : " << parent->decision << endl;
        return;
    }
...
int main(){
    ...
    //vector <int> counts;
    //count_class_labels(training_data, counts, indexes);
    //cout << counts.size() << endl;
    //print_counts(counts);
    if(homogeneous_class_labels(training_data, indexes))
    ...
    cout << endl << "Diff count : " << diff_count << endl;
