...
int main(int argc, char *argv[])
{
	...
	string strShapeFilename = "shapes";
	int nXResolution = 0;
	int refraction_on = 0;
	int shadow_on = 0;
	int reflection_on = 0;
	...
	if ( argc < 10){
		...
		if ( argc > 10)
            strShapeFilename = argv[10];

	}
	RayTracer rt(strInputFilename, strOutFilename, strShapeFilename, nXResolution, nYResolution, shadow_on, reflection_on, refraction_on, depth_of_field_on, super_sampling);
	rt.trace_rays();
	return 0;
}
